...
parser.add_argument('--val_subset', type=str, default='subset5',
                    help='the directory name of the validation set in data_path, the rest of directory will be used as training set')
...
def train(net, train_dl, loss_func, opt):
    ...
    for step, (ct, seg) in enumerate(train_dl):
        ...
        elif args.model_name == 'SC_UNet' or args.model_name == 'Cascaded_UNet':
            outputs_1, outputs_2 = net(ct.cuda())
    ...
    for root, dirs, files in os.walk(validate_path):
        if len(files) and args.val_subset in root:
            ...
            if args.HU_upper_threshold == abs(args.HU_lower_threshold):
                img_arr = img / args.HU_upper_threshold
            else:
        ...
        # if len(files) and 'subset1' in root:
            ...
            with torch.no_grad():
                ...
                for i in range(new_img_arr.shape[0]):
                    slice = new_img_arr[i, :, :]
                    slice = torch.FloatTensor(slice).cuda().unsqueeze(dim=0).unsqueeze(dim=0)
                    # print(torch.min(slice), torch.max(slice))
